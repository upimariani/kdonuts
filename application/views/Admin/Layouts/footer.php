...
<script>
    ...
    foreach ($produk as $key => $value) {
        $produk_terlaris[] = $value->nama_barang;
        $jml_terjual[] = $value->jml;
    }
    ?>
    var ctx = document.getElementById('grafik_qty');
    var grafik = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: <?= json_encode($produk_terlaris) ?>,
            datasets: [{
                label: 'Grafik Analisis Produk Terjual',
                data: <?= json_encode($jml_terjual) ?>,
                backgroundColor: [
